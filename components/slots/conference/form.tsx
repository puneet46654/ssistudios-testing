...
import { ConferenceBookingFormData } from '@/app/slots/conference/page';

interface BookingFormProps {
  formData: ConferenceBookingFormData;
  selectedSlot: string;
  ...
  return (
    <form onSubmit={onSubmit} className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full max-w-4xl mx-auto space-y-5 px-3 sm:px-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-white border border-slate-100 p-5 rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] gap-4">
        <div>
          <span className="text-[10px] font-extrabold text-purple-600 uppercase tracking-wider block mb-1">Locked Target Slot</span>
          <div className="flex flex-wrap items-center gap-2.5">
            <span className="text-base sm:text-lg font-black text-slate-900">{formData.bookingDate}</span>
            <span className="bg-purple-50 text-purple-700 text-xs font-black px-3 py-1 rounded-full">{selectedSlot}</span>
          </div>
        </div>
        <button 
          type="button" 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <select 
                ...
                onChange={onChange} 
                className="w-24 sm:w-28 bg-slate-50/80 border border-slate-200 rounded-2xl px-3 py-3 text-xs sm:text-sm font-bold text-slate-800 outline-none focus:border-purple-500 focus:bg-white cursor-pointer shadow-sm"
              >
              ...
              <input 
                ...
                onChange={onChange} 
                className="flex-1 bg-slate-50/80 border border-slate-200 rounded-2xl px-4 py-3 text-xs sm:text-sm font-medium text-slate-800 outline-none focus:border-purple-500 focus:bg-white shadow-sm" 
              />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex gap-2">
              <select 
                ...
                onChange={onChange} 
                className="w-28 sm:w-32 bg-slate-50/80 border border-slate-200 rounded-2xl px-2 sm:px-3 py-3 text-xs sm:text-sm font-bold text-slate-800 outline-none focus:border-purple-500 focus:bg-white cursor-pointer shadow-sm"
              >
          ...
          <div className="space-y-1.5">
            ...
            <input 
              ...
              name="email" 
              required 
              placeholder="delegate@institution.com"
          ...
          <div className="space-y-1.5">
            ...
            <input 
              ...
              name="location" 
              required 
              placeholder="e.g. Main Convention Center"
      ...
      <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-2">
        ...
        <button 
          ...
          className="w-full sm:w-auto sm:min-w-[220px] px-8 py-3.5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-2xl text-xs sm:text-sm transition-all cursor-pointer disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-purple-600/25 flex items-center justify-center gap-2"
        >
          {isSubmitting ? (
            <>
